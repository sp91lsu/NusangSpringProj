	...
	<div id="chat-page">
		<div class="chat-container">
			<div class="chat-header">
				<input type="hidden" id="matchUserNickname"
					value="${chatRoom.getMatchedUser().nickname }" />
			...
			<ul id="messageArea" style="background-color: #B2C7D9">
				<c:forEach var="message" items="${ chatRoom.messageList}">
					...
					<c:set var="view_cnt"
						value="${message.view_cnt > 0 ? message.view_cnt : '' }"></c:set>
					<c:choose>
			...
			<input type="hidden" id="nickname"
				value="${principal.user.nickname }"> <input type="hidden"
				id="userno" value="${principal.user.userno }"> <input
				type="hidden" id="chatRoomTopic" value="${chatRoom.topic}">
			...
			<form id="leaveForm" name="leaveForm">
				<button id="chat_btn_common" type="button" onclick="history.back()"
					class="primary">돌아가기</button>
			</form>
		</div>
	</div>
	<script
