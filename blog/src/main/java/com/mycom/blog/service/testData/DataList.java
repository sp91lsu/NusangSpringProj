...
//	아이디(중복 불가)
	String uniqID(int numberth) {
		String[] engNameArr = engNames.split(",");
		int len = engNameArr.length;
		return engNameArr[numberth%len]+decTo62(numberth/len);
	}
		String engNames = "Aaliyah," + "Abby," + "Abigail," + "Ada," + "Adalee," + "Adaline," + "Adalyn,"
...
//	성명 랜덤
	...
	String randName(GenderType gen) {
		String name = null;
		String rand2ndName = gen==GenderType.MALE ? rand2ndName_M() : rand2ndName_W();
		name = rand1stName()+rand2ndName;
		return name;
	}
		...
		// 여자 이름 랜덤
		String rand2ndName_W() {
			String secondN = null;
			String[] secondNameArr_W = secondNames_W.split(",");
			secondN = oneOf(secondNameArr_W);
			return secondN;
		}
...
//	닉네임(중복 불가)
	String uniqNic(int numberth) {
		String[] nicNameArr = nicNames.split(",");
		int len = nicNameArr.length;
		return nicNameArr[numberth%len]+decTo62(numberth/len);
	}
...
//	regdate 랜덤
	Timestamp randRD(int rangeOfPastDays) {
...
//		}
		for (int i = 0; i < 100; i++) {
			System.out.println(s.randRD(200));
		}
	}
}
